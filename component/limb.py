...
def build(side):
    ...
    boneDict = {
    'clavicle_{}'.format(side) : utils.get_bone_location(gArm, 'clavicle_{}'.format(side)), 
    'upper_arm_{}'.format(side) : utils.get_bone_location(gArm, 'upper_arm_{}'.format(side)), 
    'lower_arm_{}'.format(side) : utils.get_bone_location(gArm, 'lower_arm_{}'.format(side)), 
    'wrist_{}'.format(side) : utils.get_bone_location(gArm, 'wrist_{}'.format(side)), 
    }
    defBones = []
    conBones = []
    hlpTwistBones = []
    ...
    head = boneDict[keyList[0]][0]
    tail = boneDict[keyList[0]][1]
    roll = boneDict[keyList[0]][2]
    bone = utils.addBone(arm, name='def.clavicle.{}.001'.format(side), head=head, tail=tail, roll=roll)
    ...
    for i in range(1,3):
        ...
        if i == 1:
            upperArmLocations.extend([location1, location2, location3, location4, location5])
        else:
            lowerArmLocations.extend([location1, location2, location3, location4, location5])

    for index, location in enumerate(upperArmLocations):
        if location != upperArmLocations[-1]:
            head = upperArmLocations[index]
            tail = upperArmLocations[index+1]
            bone = utils.addBone(arm, name='def.upper_arm.{}.00{}'.format(side, str(index+1)), head=head, tail=tail, roll=roll)
            defBones.append(bone)

    for index, location in enumerate(lowerArmLocations):
        if location != lowerArmLocations[-1]:
            head = lowerArmLocations[index]
            tail = lowerArmLocations[index+1]
            bone = utils.addBone(arm, name='def.lower_arm.{}.00{}'.format(side, str(index+1)), head=head, tail=tail, roll=roll)
            defBones.append(bone)

    head = boneDict[keyList[3]][0]
    tail = boneDict[keyList[3]][1]
    roll = boneDict[keyList[3]][2]
    bone = utils.addBone(arm, name='def.wrist.{}.001'.format(side), head=head, tail=tail, roll=roll)
    defBones.append(bone)

    # con, hlp twist bones
    for i, eBone in enumerate(defBones):
        # con bones
    ...
    # mst bone
    mstBone = utils.addBone(arm, name='mst.arm.{}.001'.format(side), head=boneDict['upper_arm_{}'.format(side)][0], tail=boneDict['upper_arm_{}'.format(side).format(side)][1], roll=0)
    ...
    i=1
    # tweak prt bones
    for eBone in [defBones[3], defBones[5], defBones[7]]:
        tweakParentBone = utils.duplicate_bone(arm, eBone, 'prt.arm_fk_tweak.{}.00{}'.format(side, str(i)), scale = .4)
        tweakParentBones.append(tweakParentBone)
        i += 1
    # ik prt bone
    
    ikPrtBone = utils.addBone(arm, name='prt.arm_ik.{}.001'.format(side), head=boneDict['upper_arm_{}'.format(side)][0], tail=boneDict['upper_arm_{}'.format(side)][1], roll=roll)
    ikPrtBone.length = ikPrtBone.length*.4
    # pole vector prt bone
    ikPolePrtBone = utils.addBone(arm, name='prt.arm_pole_vector.{}.001'.format(side), head=boneDict['upper_arm_{}'.format(side)][0], tail=boneDict['wrist_{}'.format(side)][1], roll=roll)
    '''
    avgHead = ((boneDict['upper_arm_{}'.format(side)][0][0]+boneDict['wrist_{}'.format(side)][0][0])/2, (boneDict['upper_arm_{}'.format(side)][0][1]+boneDict['wrist_{}'.format(side)][0][1])/2, (boneDict['upper_arm_{}'.format(side)][0][2]+boneDict['wrist_{}'.format(side)][0][2])/2)
    avgTail = ((boneDict['upper_arm_{}'.format(side)][1][0]+boneDict['wrist_{}'.format(side)][1][0])/2, (boneDict['upper_arm_{}'.format(side)][1][1]+boneDict['wrist_{}'.format(side)][1][1])/2, (boneDict['upper_arm_{}'.format(side)][1][2]+boneDict['wrist_{}'.format(side)][1][2])/2)
    ikPolePrtBone = utils.addBone(arm, name='prt.arm_pole_vector.{}.001'.format(side), head=avgHead, tail=avgTail, roll=roll)
    ikPolePrtBone.length = ikPolePrtBone.length*.4
    '''
    # tweak control bones
    i=1
    for eBone in tweakParentBones:
        tweak = utils.duplicate_bone(arm, eBone, 'ctl.arm_fk_tweak.{}.00{}'.format(side,str(i)), scale = 1)
        tweaks.append(tweak)
        i += 1
    # fk bones
    ...
    # ik bones
    for eBone in [hlpBones[1], hlpBones[3]]:
        #main
        baseName = eBone.name.split('.')[1] 
    ...
    # pole vector
    head = (boneDict['lower_arm_{}'.format(side)][0][0], boneDict['lower_arm_{}'.format(side)][0][1]+1, boneDict['lower_arm_{}'.format(side)][0][2])
    tail = (head[0]+1, head[1], head[2])
    poleVector = utils.addBone(arm, name='ctl.arm_pole_vector.{}.001'.format(side), head=head, tail=tail, roll=roll)
    poleVectorSub = utils.addBone(arm, name='ctl.arm_pole_vector_sub.{}.001'.format(side), head=head, tail=tail, roll=roll)
    poleVectorSub.length = poleVectorSub.length*.7
    # property control bone
    head = (boneDict['wrist_{}'.format(side)][0][0], boneDict['wrist_{}'.format(side)][0][1], boneDict['wrist_{}'.format(side)][0][2] + 0.1)
    tail = (boneDict['wrist_{}'.format(side)][1][0], boneDict['wrist_{}'.format(side)][1][1], boneDict['wrist_{}'.format(side)][1][2] + 0.1)
    armControl = utils.addBone(arm, name='ctl.arm_controls.{}.001'.format(side), head=head, tail=tail, roll=roll)
    ...
    armControlName = armControl.name
    ...
    for bone in tweaks:
        ...
        bone.layers[utils.boneLayerDict['god']] = False
    # armControl
    armControl.layers[utils.boneLayerDict['fk']] = True
    armControl.layers[utils.boneLayerDict['ik']] = True
    armControl.layers[utils.boneLayerDict['god']] = False
    # mstBone
    ...
    for i, bone in enumerate(hlpTwistBones):
        if i < 4:
            bone.parent = bpy.data.objects['buildArmature'].data.edit_bones['hlp.upper_arm.{}.001'.format(side)]
        elif i > 3:
            bone.parent = bpy.data.objects['buildArmature'].data.edit_bones['hlp.lower_arm.{}.001'.format(side)]
        elif i == (len(hlpTwistBones)-1):
            bone.parent = bpy.data.objects['buildArmature'].data.edit_bones['hlp.wrist.{}.001'.format(side)]
    # fkBones
    i = 0
    ...
    for i, bone in enumerate(tweakParentBones):
        if i == 0:
            ...
            bone.parent = hlpBones[2]
    # armControl
    armControl.parent = hlpBones[2]

    bpy.ops.object.mode_set(mode='POSE')
    # custom properties
    utils.add_custom_property_to_pose_bone(arm, boneName=armControlName, propertyName="IKFK_switch", default=1.0, min=0.0, max=1.0 )
    utils.add_custom_property_to_pose_bone(arm, boneName=armControlName, propertyName="IKPoleVector_follow_god_wrist", default=1.0, min=0.0, max=1.0 )
    utils.add_custom_property_to_pose_bone(arm, boneName=armControlName, propertyName="IK_stretch", default=0.0, min=0.0, max=1.0 )
    utils.add_custom_property_to_pose_bone(arm, boneName=armControlName, propertyName="auto_bend_switch", default=0.0, min=0.0, max=1.0 )
    utils.add_custom_property_to_pose_bone(arm, boneName=armControlName, propertyName="volume_preservation", default=1.0, min=0.0, max=1.0 )
    ...
    # con constraints
    pBone = arm.pose.bones[conBoneNames[0]]
    copyTrans = pBone.constraints.new('COPY_TRANSFORMS')
    copyTrans.target = arm
    copyTrans.subtarget = hlpBoneNames[0]
    i=0
    for boneName in conBoneNames:
        if i != 0:
            pBone = arm.pose.bones[boneName]
            copyTrans = pBone.constraints.new('COPY_TRANSFORMS')
            copyTrans.target = arm
            copyTrans.subtarget = hlpTwistBoneNames[i-1]
        if i != len(hlpTwistBoneNames):
            stretchTO = pBone.constraints.new('STRETCH_TO')
            stretchTO.target = arm
            stretchTO.subtarget = hlpTwistBoneNames[i]
            drv = utils.add_driver(pBone.constraints['{}'.format(stretchTO.name)], 'bulge', func = 'var')
            utils.add_driver_variable(drv, varName='var', target=arm, dataPath='pose.bones["ctl.arm_controls.{}.001"]["volume_preservation"]'.format(side))
        i+=1
    # hlp twist constraints
    for i,boneName in enumerate(hlpTwistBoneNames):
        if 1 < i < 4:
            subtarget = 'hlp.upper_arm.{}.001'.format(side)
            transSubtarget = 'ctl.arm_fk_tweak.{}.001'.format(side)
        elif 3 < i < 8 :
            subtarget = 'hlp.lower_arm.{}.001'.format(side)
            transSubtarget = 'ctl.arm_fk_tweak.{}.003'.format(side)
        else:
            subtarget = 'hlp.wrist.{}.001'.format(side)
        if i in ( 3, 5, 7):
            # copy location
            ...
            if i == 2:
                coptTransSubTarget = 'ctl.arm_fk_tweak.{}.001'.format(side)
            elif i == 4:
                coptTransSubTarget = 'ctl.arm_fk_tweak.{}.002'.format(side)
            copyTrans = arm.pose.bones[boneName].constraints.new('COPY_TRANSFORMS')
            copyTrans.target = arm
    ...
    for i,boneName in enumerate(hlpBoneNames):
        ...
        copyTrans.target = arm
        if i == 0: 
            copyTrans.subtarget = 'ctl.clavicle_fk_sub.{}.001'.format(side)
        if i == 1: 
            copyTrans.subtarget = 'ctl.upper_arm_fk_sub.{}.001'.format(side)
        if i == 2: 
            copyTrans.subtarget = 'ctl.lower_arm_fk_sub.{}.001'.format(side)
        if i == 3: 
            copyTrans.subtarget = 'ctl.wrist_fk_sub.{}.001'.format(side)
        # copy Transform ik
        if i != 0: 
            copyTrans = pBone.constraints.new('COPY_TRANSFORMS')
            ...
            copyTrans.target = arm
        if i == 1: 
            copyTrans.subtarget = 'hlp.upper_arm_ik_stretch.{}.001'.format(side)
        if i == 2: 
            copyTrans.subtarget = 'hlp.lower_arm_ik_stretch.{}.001'.format(side)
        if i == 3: 
            copyTrans.subtarget = 'hlp.wrist_ik_stretch.{}.001'.format(side)
        # copy transforms driver   
        drv = utils.add_driver(pBone.constraints['{}'.format(copyTrans.name)], 'influence', func = 'var')
        utils.add_driver_variable(drv, varName='var', target=arm, dataPath='pose.bones["ctl.arm_controls.{}.001"]["IKFK_switch"]'.format(side))
        # copy Location tweak
        if i == 2:
            copyLoc = pBone.constraints.new('COPY_LOCATION')
            copyLoc.name = 'Copy Location Tweak'
            copyLoc.target = arm
            copyLoc.subtarget = 'hlp.upper_arm_ik_stretch.{}.001'.format(side)
            copyLoc.subtarget = 'ctl.arm_fk_tweak.{}.002'.format(side)
        # Y rotation transformation
            transformYRot = arm.pose.bones[boneName].constraints.new('TRANSFORM')
            transformYRot.name = 'Transformation Y Rotation'
            transformYRot.target = arm
            transformYRot.subtarget = 'ctl.arm_fk_tweak.{}.002'.format(side)
            transformYRot.use_motion_extrapolate = True
            transformYRot.owner_space = 'LOCAL'  
            transformYRot.target_space = 'LOCAL' 
            transformYRot.map_from = 'ROTATION'
    ...
    for boneList in [noStretchIkmBoneNames, ikmBoneNames]:
        for i in range(2):
            if boneList == ikmBoneNames:
                boneName = boneList[i]
                arm.pose.bones[boneName].ik_stretch = 0.1
        boneName = boneList[1]
        IK = arm.pose.bones[boneName].constraints.new('IK')
        IK.target = arm
        IK.subtarget  = 'ctl.wrist_ik_sub.{}.001'.format(side)
        IK.pole_target = arm
        IK.pole_subtarget = 'ctl.arm_pole_vector_sub.{}.001'.format(side)
    ...
    # copy transform
        ...
        copyTrans = arm.pose.bones[boneName].constraints.new('COPY_TRANSFORMS')
        copyTrans.target = arm
        copyTrans.subtarget = 'ctl.wrist_ik_sub.{}.001'.format(side)
    ...
    for boneName, subtarget in zip(hlpIkStretchBoneNames, ikmBoneNames):
        ...
        if i in (0,1):
            ...
            copyTrans.subtarget = subtarget
            drv = utils.add_driver(pBone.constraints['{}'.format(copyTrans.name)], 'influence', func = 'var')
            utils.add_driver_variable(drv, varName='var', target=arm, dataPath='pose.bones["ctl.arm_controls.{}.001"]["IK_stretch"]'.format(side))
        else:
            # copy rotation/scale
            subtarget = 'ctl.wrist_ik_sub.{}.001'.format(side)
    ...
    copyTrans = pBone.constraints.new('COPY_TRANSFORMS')
    copyTrans.target = arm
    copyTrans.subtarget = 'ctl.upper_arm_ik_sub.{}.001'.format(side)
    # ikPolePrtBone
    ...
    copyTrans.subtarget = 'ctl.wrist_ik_sub.{}.001'.format(side)
    drv = utils.add_driver(pBone.constraints['{}'.format(copyTrans.name)], 'influence', func = 'var')
    utils.add_driver_variable(drv, varName='var', target=arm, dataPath='pose.bones["ctl.arm_controls.{}.001"]["IKPoleVector_follow_god_wrist"]'.format(side))
    
    copyTrans = pBone.constraints.new('COPY_LOCATION')
    copyTrans.target = arm
    copyTrans.subtarget = 'mst.arm.{}.001'.format(side)
    ...
    copyRot.target = arm
    copyRot.influence = .5
    copyRot.subtarget = 'ctl.lower_arm_fk_sub.{}.001'.format(side)

    copyRot = pBone.constraints.new('COPY_ROTATION')
    copyRot.name = 'Copy Rotation IK'
    copyRot.target = arm
    copyRot.influence = .5
    copyRot.subtarget = 'hlp.upper_arm_ik_stretch.{}.001'.format(side)
    ...
    for i, boneName in enumerate(tweakParentBoneNames):
        if i in (0,2):
            ...
            if i == 2:
                utils.add_driver_variable(drv, varName='var', target=arm, dataPath='pose.bones["ctl.arm_fk_tweak.{}.003"].location[0]'.format(side))
            else:
                utils.add_driver_variable(drv, varName='var', target=arm, dataPath='pose.bones["ctl.arm_fk_tweak.{}.001"].location[0]'.format(side))
            utils.add_driver_transform_variable(drv, varName='auto', target=arm, bone_target='hlp.lower_arm.{}.001'.format(side), transform_type='ROT_Z', transform_space = 'LOCAL_SPACE')
            utils.add_driver_variable(drv, varName='scale', target=arm, dataPath='pose.bones["ctl.arm_fk_tweak.{}.002"].scale[1]'.format(side))
            utils.add_driver_variable(drv, varName='auto_switch', target=arm, dataPath='pose.bones["ctl.arm_controls.{}.001"]["auto_bend_switch"]'.format(side))
    ...
    for i, boneName in enumerate(hlpBoneNames):
        if i in (1,2):
            ...
            if i == 2:
                ...
                utils.add_driver_variable(drv, varName='var', target=arm, dataPath='pose.bones["ctl.arm_fk_tweak.{}.001"].location[0]'.format(side))
            utils.add_driver_variable(drv, varName='scale', target=arm, dataPath='pose.bones["ctl.arm_fk_tweak.{}.002"].scale[1]'.format(side))
            utils.add_driver_rotation_diff_variable(drv, varName='auto', target=arm, bone_target1='hlp.arm.{}.001'.format(side), bone_target2='hlp.arm.{}.002'.format(side))
            utils.add_driver_variable(drv, varName='auto_switch', target=arm, dataPath='pose.bones["ctl.arm_controls.{}.001"]["auto_bend_switch"]'.format(side))
            ...
            if i == 2:
                utils.add_driver_variable(drv, varName='var', target=arm, dataPath='pose.bones["ctl.arm_fk_tweak.{}.003"].location[2]'.format(side))
            else:
                utils.add_driver_variable(drv, varName='var', target=arm, dataPath='pose.bones["ctl.arm_fk_tweak.{}.001"].location[2]'.format(side))
...
class LimbGuides(Operator):
    ...
    def execute(self, context):
        boneDict = boneDict = utils.get_bone_dict_data(fileName='limb_guide_data.json') 
        utils.import_shapes()
        ...
        for bone in boneDict.keys():
            ...
            utils.add_custom_property_to_pose_bone(arm, boneName=editBone.name, propertyName="Component", default='limb' )
...
class BUILD_OT_Limb(Operator):
    """Creates operator for drop-down menu of rig components and templates"""
    bl_idname = "build.limb"
    bl_label = "Limb Build"

    def execute(self, context):
        build('L')
        boneNames = [bone.name for bone in bpy.data.armatures['guideArmature'].bones]
        if 'clavicle_R' in boneNames:
            build('R')
        return {"FINISHED"}

classes = [LimbGuides, BUILD_OT_Limb]